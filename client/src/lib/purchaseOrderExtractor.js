 ...
 * @property {{ name?: string, site?: string, contactPerson?: string, address?: string, contacts?: { primary?: { name?: string, phone?: string }, secondary?: { name?: string, phone?: string } } }} [vendor]
...
export function extractPurchaseOrderFields(rawText) {
  ...
  const contactLine = lines.find((l) => /^Contact\s*Person\s*:\s*/i.test(l));
  if (contactLine) out.vendor.contactPerson = stripLabel(contactLine, /^Contact\s*Person\s*:\s*(.+)$/i);

  const addrIdx = lines.findIndex((l) => /^Address\s*:\s*/i.test(l));
  if (addrIdx !== -1) {
    const addrParts = [];
    const first = stripLabel(lines[addrIdx], /^Address\s*:\s*(.+)$/i);
    ...
    if (addrParts.length) out.vendor.address = addrParts.join(' ');
  }
